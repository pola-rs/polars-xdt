...
def get_result(
    ...
    by: str | pl.Series,
    weekend: list[str],
    holidays: list[dt.date] | None,
    roll: Literal['forward', 'backward', 'raise'],
) -> dt.date:
    if dtype == pl.Date:
        result = (
            pl.DataFrame({"ts": [date]})
            .select(plb.col("ts").bdt.offset_by(by=by, weekend=weekend, holidays=holidays, roll=roll))["ts"]  # type: ignore[arg-type]
            .item()
        )
    else:
        try:
            result = (
                ...
                .select(
                    ...
                    .bdt.offset_by(by=by, weekend=weekend, holidays=holidays, roll=roll)  # type: ignore[attr-defined]
                    .dt.date()
                )["ts"]
                .item()
            )
            ...
            assert (
                "non-existent" in str(exp)
                or "ambiguous" in str(exp)
                or (
                    "is not a business date" in str(exp)
                    and roll == "raise"
                )
            )
            reject()
...
@given(
    date=st.dates(min_value=dt.date(1969, 1, 1), max_value=dt.date(1971, 1, 1)),
    n=st.integers(min_value=-30, max_value=30),
    weekend=st.lists(
    ...
    holidays=st.lists(
        st.dates(min_value=dt.date(1969, 1, 1), max_value=dt.date(1971, 1, 1)),
        min_size=1,
        max_size=300,
    ),
...
def test_against_np_busday_offset(
    ...
    expected = np.busday_offset(
        date, n, weekmask=weekmask, holidays=holidays
    )
...
def test_against_np_busday_offset_roll(
    date: dt.date,
    n: int,
    weekend: list[str],
    holidays: list[dt.date],
