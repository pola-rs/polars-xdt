...
def test_format_localized_datetime(time_unit: TimeUnit, expected_us: str, expected_ukr: str) -> None:
    ...
    result = df.select(result=xdt.format_localized('date_col', "%A, %d %B %Y %H:%M:%S%.f", 'en_US'))['result']
    assert result[0] == expected_us
    result = df.select(result=xdt.format_localized('date_col', "%A, %d %B %Y %H:%M:%S%.f", 'uk_UA'))['result']
...
def test_format_localized_date() -> None:
    ...
    result = df.select(result=xdt.format_localized('date_col', "%A, %d %B %Y", 'en_US'))['result']
    assert result[0] == 'Saturday, 24 August 2024'
    assert result[1] == 'Tuesday, 01 October 2024'
    result = df.select(result=xdt.format_localized('date_col', "%A, %d %B %Y", 'uk_UA'))['result']
...
def test_tz_aware() -> None:
    ...
    result = (df.select(result=xdt.format_localized('date_col', "%A, %d %B %Y %z", "uk_UA")))
...
def test_pre_epoch(time_unit: TimeUnit) -> None:
    ...
    result = (df.select(result=xdt.format_localized('date_col', "%A, %d %B %Y %z", "en_US")))
