...
os.system('cp docs/_build/html/* ../docs-polars-xts/ -r')
subprocess.run(['git', 'add', '.'], cwd='../docs-polars-xts')
subprocess.run(['git', 'commit', '-m', '\"new version\"', '--allow-empty'], cwd='../docs-polars-xts')
subprocess.run(['git', 'push', 'origin', 'HEAD'], cwd='../docs-polars-xts')
...
with open("polars_xts/__init__.py", "r", encoding="utf-8") as f:
    content = f.read()
content = content.replace(
    f'__version__ = "{old_version}"', f'__version__ = "{version}"'
)
with open("polars_xts/__init__.py", "w", encoding="utf-8") as f:
    f.write(content)
